...
import { ExternalLink, ArrowLeft, ShieldCheck, MapPin, Users, Briefcase, ChevronLeft, ChevronRight } from "lucide-react";

const slides = [
  {
    img: "/images/photos/slam-scanning.jpg",
    eyebrow: "حلول مساحية متكاملة | المملكة العربية السعودية",
    h1Line1: "المعدات المناسبة.",
  ...
  },
  {
    ...
    eyebrow: "الصيانة والمعايرة",
    h1Line1: "حافظ على دقة معداتك",
    h1Line2: "وجاهزيتها الميدانية.",
    subtitle:
      "صيانة ومعايرة معتمدة وخدمات ما بعد البيع لمعداتك المساحية. باقات متاحة لأساطيل المعدات في جميع الفروع الخمسة.",
    ctaPrimary: { label: "عرض باقات الخدمة", href: "/ar/maintenance-calibration", external: false },
    ctaSecondary: { label: "تواصل معنا", href: "/ar/contact", external: false },
  },
];

const trustBadges = [
  { icon: <Users size={14} />, text: "+3,000 عميل B2B" },
  { icon: <MapPin size={14} />, text: "5 فروع في المملكة" },
  { icon: <ShieldCheck size={14} />, text: "ISO 9001:2015" },
  { icon: <Briefcase size={14} />, text: "+2,000 مشروع" },
];

export function BannerHeroSectionAr() {
    ...
    >
      ...
      {slides.map((s, i) => (
        ...
        >
          ...
          />
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(260deg, rgba(13,22,30,0.85) 0%, rgba(13,22,30,0.6) 50%, rgba(13,22,30,0.25) 100%)" }} />
        </div>
      ))}
      ...
      <div className="se-container" style={{ position: "relative", zIndex: 3, flex: 1, display: "flex", alignItems: "center", padding: "120px 0 80px" }}>
        <div style={{ maxWidth: 620 }}>
          ...
          >
            ...
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", color: "var(--se-teal)" }}>
              {slide.eyebrow}
            </span>
          </div>
          ...
          <p
            key={`sub-${active}`}
            style={{
              color: "rgba(255,255,255,0.72)", fontSize: 17, lineHeight: 1.8,
          ...
          >
            {slide.ctaPrimary.external ? (
              ...
              <Link href={slide.ctaPrimary.href}
                ...
                <ArrowLeft size={15} /> {slide.ctaPrimary.label}
              </Link>
            )}
      ...
      {/* Slide counter */}
      <div style={{ position: "absolute", bottom: 38, left: "max(24px, calc(50% - 580px))", zIndex: 3, fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.35)", letterSpacing: "0.1em" }}>
