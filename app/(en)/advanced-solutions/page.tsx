...
export const metadata: Metadata = {
  title: "Advanced Solutions | 3D Scanning, Drone Mapping & More | Surveying Experts",
  description:
...
function Eyebrow({ children, teal }: { children: React.ReactNode; teal?: boolean }) {
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 10, marginBottom: "1.25rem" }}>
      <span style={{ display: "inline-block", width: 24, height: 2, background: teal ? "var(--se-teal)" : "var(--accent)", borderRadius: 2, flexShrink: 0 }} />
      <span style={{ fontFamily: "var(--font-sans)", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.2em", color: teal ? "var(--se-teal)" : "var(--accent)" }}>
        {children}
      </span>
    </div>
  );
  ...
  return (
    <>
      ...
      <section style={{ background: "#0d1e2a", position: "relative", overflow: "hidden", padding: "100px 0 80px", minHeight: 440 }}>
        <div aria-hidden style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, opacity: 0.7 }} />
        <div className="se-container" style={{ position: "relative" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 40, alignItems: "start" }} className="kit-2col">
            <div>
              <Eyebrow teal>Advanced Solutions</Eyebrow>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                <Link href="#advanced-solutions-detail"
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20, paddingTop: 20 }} className="kit-layers">
              {layers.map((l) => (
                <div key={l.label} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 36, height: 36, borderRadius: "50%", border: "1.5px solid rgba(34,167,168,0.5)", color: "var(--se-teal)", display: "grid", placeItems: "center", flexShrink: 0 }}>
                    {l.icon}
                  </div>
                  <div>
                    ...
                    <div style={{ fontFamily: "var(--font-sans)", fontSize: 12, color: "#ACC9DC" }}>{l.sub}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Why It Matters */}
      <section style={{ background: "#0d1e2a", position: "relative", overflow: "hidden", padding: "72px 0" }}>
        <div className="se-container" style={{ position: "relative" }}>
          <SecHead label="Why It Matters" title="Why Advanced Surveying Solutions Matter" light centered teal />
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
            {awareness.map((a) => (
              <div key={a.title} style={{ border: "1px solid rgba(34,167,168,0.2)", borderRadius: "var(--radius-lg)", padding: "22px 18px" }}>
                <div style={{ width: 40, height: 40, borderRadius: "var(--radius-md)", color: "var(--se-teal)", display: "grid", placeItems: "center", marginBottom: 14 }}>
                  {a.icon}
                </div>
                <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 15, color: "#fff", marginBottom: 8 }}>{a.title}</div>
                <div style={{ fontFamily: "var(--font-sans)", fontSize: 13, lineHeight: 1.6, color: "#C5DAE6" }}>{a.text}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: "96px 0" }}>
        <div className="se-container">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }} className="kit-2col">
            <div style={{ borderRadius: "var(--radius-xl)", padding: "32px 28px", border: "1px solid var(--se-blue-light)" }}>
            ...
            <div style={{ borderRadius: "var(--radius-xl)", padding: "32px 28px", border: "1px solid rgba(34,167,168,0.3)" }}>
              <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 20, color: "var(--se-teal-dark)", marginBottom: 20 }}>Advanced Solutions</div>
      ...
      {/* Accordion */}
      <section id="advanced-solutions-detail" style={{ background: "#0d1e2a", padding: "88px 0", position: "relative", overflow: "hidden" }}>
        <div className="se-container" style={{ position: "relative" }}>
          <SecHead label="Our Advanced Solution Areas" title="Advanced Technologies We Support" subtitle="We provide access, consultation, equipment supply, operation guidance, and after-sales support for each area." light centered teal />
          <HighSolutionAccordion />
        </div>
      </section>

      {/* Support Process — improved timeline visualization */}
      <section style={{ position: "relative", overflow: "hidden", padding: "96px 0", backgroundImage: "radial-gradient(circle at 1px 1px, rgba(53,80,97,0.10) 1px, transparent 0)", backgroundSize: "22px 22px" }}>
        <div className="se-container">
          ...
          {/* Timeline — horizontal with connectors */}
          <div style={{ position: "relative", marginTop: 16 }}>
            {/* Connecting line */}
            <div aria-hidden style={{ position: "absolute", top: 30, left: "calc(10% + 25px)", right: "calc(10% + 25px)", height: 2, background: "linear-gradient(90deg, var(--se-teal) 0%, rgba(34,167,168,0.2) 100%)", zIndex: 0 }} />
                ...
                <div key={s.title} style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
                  ...
                  <div style={{ position: "relative", marginBottom: 16 }}>
                    <div style={{
                      ...
                      background: i === 0 ? "var(--se-teal)" : "var(--se-white)",
                      border: "2px solid var(--se-teal)",
                      color: i === 0 ? "#fff" : "var(--se-teal)",
                      display: "grid", placeItems: "center", flexShrink: 0,
                      boxShadow: "0 4px 16px rgba(34,167,168,0.25)",
                    ...
                    <div style={{
                      ...
                      width: 20, height: 20, borderRadius: "50%", border: "2px solid var(--se-gray-50)",
                      display: "grid", placeItems: "center",
                    }}>
                      <span style={{ fontFamily: "var(--font-sans)", fontWeight: 800, fontSize: 8, color: "var(--se-blue-dark)" }}>{i + 1}</span>
                    </div>
                  </div>
                  <div style={{ fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: 13, color: "var(--text-strong)", marginBottom: 6, lineHeight: 1.3 }}>{s.title}</div>
                  <div style={{ fontFamily: "var(--font-sans)", fontSize: 12, color: "var(--text-muted)", lineHeight: 1.55 }}>{s.text}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ textAlign: "center", marginTop: 48 }}>
            <Link href="/contact"
              style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 24px", color: "#fff", background: "var(--se-teal)", fontWeight: 700, fontSize: 15, borderRadius: "var(--radius-md)", textDecoration: "none" }}>
      ...
      {/* CTA */}
      <section style={{ background: "#0d1e2a", position: "relative", overflow: "hidden", padding: "80px 0" }}>
        <div className="se-container" style={{ position: "relative", textAlign: "center" }}>
          ...
          <p style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-md)", color: "#CCDFE9", marginBottom: "2rem" }}>Tell us about your project and our team will recommend the right advanced solution.</p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 14, justifyContent: "center" }}>
            <Link href="/contact"
              ...
              Talk to an Advanced Solutions Specialist <ArrowRight size={16} />
            </Link>
            <Link href="/contact"
